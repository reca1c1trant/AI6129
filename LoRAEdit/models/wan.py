...
class WanPipeline(BasePipeline):
    ...
    def load_diffusion_model(self):
        ...
        if transformer_path := self.model_config.get('transformer_path', None):
            ...
            print(f'[TIMING] init_empty_weights: {_time.time()-t_init:.1f}s')
            # Memory-efficient loading: process one shard at a time
            # Checkpoints are fp32, use GPU for fast dtype conversion
            import gc
            from safetensors.torch import load_file as st_load_file
            param_names = {name for name, _ in self.transformer.named_parameters()}
            shards = sorted(ckpt_path.glob('diffusion_pytorch_model*.safetensors'))
            t_all_shards = _time.time()
            for shard_idx, shard in enumerate(shards):
                t_shard = _time.time()
                print(f'Loading shard {shard_idx+1}/{len(shards)}: {shard.name}')
                t0 = _time.time()
                shard_dict = st_load_file(str(shard), device='cpu')
                ...
                for key, tensor in shard_dict.items():
                    if key in param_names:
                        ...
                        target_dt = dtype if is_high_prec else transformer_dtype
                ...
                del shard_dict
                t0 = _time.time()
                self.transformer.load_state_dict(converted, strict=False, assign=True)
                t_assign = _time.time() - t0
                del converted
                gc.collect()
                torch.cuda.empty_cache()
                print(f'[TIMING] shard {shard_idx+1}: read={t_read:.1f}s convert={t_convert:.1f}s assign={t_assign:.1f}s total={_time.time()-t_shard:.1f}s')
            print(f'[TIMING] all shards total: {_time.time()-t_all_shards:.1f}s')
