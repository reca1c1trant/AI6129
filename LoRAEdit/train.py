...
if __name__ == '__main__':
    ...
    print(f'[TIMING] dataset_manager.cache(): {time.time()-_t_phase:.1f}s')
    if args.cache_only:
        quit()

    _t_phase = time.time()
    model.load_diffusion_model()
    print(f'[TIMING] load_diffusion_model(): {time.time()-_t_phase:.1f}s')

    # Fixed seed for reproducible LoRA initialization (lora_A kaiming_uniform)
    torch.manual_seed(42)

    _t_phase = time.time()
    if adapter_config := config.get('adapter', None):
        model.configure_adapter(adapter_config)
        print(f'[TIMING] configure_adapter(): {time.time()-_t_phase:.1f}s')
        is_adapter = True
    ...
    # LR scheduler: default to constant, cosine is applied later after total_steps is known
    ...
    # Apply cosine annealing if configured (needs total_steps which is now available)
    ...
    if lr_scheduler_config.get('type') == 'cosine':
        ...
        print(f'Using CosineAnnealingLR: T_max={model_engine.total_steps}, eta_min={eta_min}')
    ...
    # Rich progress bar (display only, does not affect training logic)
    _rich_ctx = None
    _rich_progress = None
    _rich_task = None
    if is_main_process() and HAS_RICH:
        ...
        _rich_task = _rich_progress.add_task(
            f"Training {config['epochs']} epochs", total=model_engine.total_steps, loss=0.0, completed=step - 1,
        )
        _rich_ctx = _rich_progress.__enter__()
    ...
    while True:
        ...
        if _rich_progress is not None and _rich_task is not None:
            _rich_progress.update(_rich_task, advance=1, loss=loss,
                                  description=f"Epoch {epoch}/{config['epochs']}")
        step += 1
    ...
    if _rich_progress is not None:
        _rich_progress.__exit__(None, None, None)
    if is_main_process():
